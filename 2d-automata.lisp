...
(defun create-2d-cellular-automata (width height
  ...
  (make-instance class
                 ...
                 :next-board-data (copy-array initial-data)))
        ...
        (loop
          ...
          do
             (loop
               ...
               when (is-on current-board-data height x-offset y-offset) do
...
(defun is-on (board height i j)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (unsigned-byte 32) height i j))
  (= 1 (2d-get board height i j)))

(defun turn-on (board height i j)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (unsigned-byte 32) height i j))
  (setf (2d-get board height i j) 1))

(defun is-off (board height i j)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (unsigned-byte 32) height i j))
  (= 0 (2d-get board height i j)))

(defun turn-off (board height i j)
  (declare (optimize (speed 3) (safety 0) (debug 0))
           (type (unsigned-byte 32) height i j))
  (setf (2d-get board height i j) 0))

(defgeneric count-neighbors (object i j)
  (:documentation "Count neighbors at board location i j"))

(defgeneric left-neighbor (object i j))
(defgeneric right-neighbor (object i j))
(defgeneric up-neighbor (object i j))
(defgeneric down-neighbor (object i j))
(defgeneric neighbors (object i j))
(defmethod neighbors ((object 2d-cellular-automata) i j)
  ...
  (with-slots (width height) object
    (let ((ip (if (> 0 (- i 1))
          ...
          (jn (if (>= j (- height 1))
                  0
                  (+ j 1))))
      (declare (type (unsigned-byte 32) ip jp in jn))
      (list (cons ip jp)
            (cons ip j)
            (cons ip jn)
            (cons i jp)
...
(defmethod count-neighbors ((object 2d-cellular-automata) i j)
  ...
  (with-slots (current-board-data width height) object
    ...
    (loop for (i . j) in (neighbors object i j)
          summing (2d-get current-board-data height i j))))

(defmethod add-current-instances ((object 2d-cellular-automata)
                                  (render-style line-render-style))
  "Draw the next row of automata data by adding translations to the instance buffer."
  (with-slots (buffers instance-count max-instances
               height width
               next-board-data current-board-idx
               current-board-data) object
        ...
        (loop
          ...
          do
             (loop
               ...
               ;; If the cell is 'on' then add a quad
               when (is-on current-board-data height x-offset y-offset) do
                 (loop for (i j) in (neighbors object x-offset y-offset) do
                   (when (is-on next-board-data height i j)
                     (sgl:fill-pointer-offset (vec3 x-float
                                                    y-float
